...
from src.utils import not_a_hacking_tool_without_ascii_art, print_help_menu
    ...
    match sys.argv[1]:
        case '-b':
            if len(sys.argv) >= 5 and sys.argv[3] == "-h":
                ...
                if not bypass.check_file(header_file):
            ...
            filename = sys.argv[2]
            if not bypass.check_file(filename):
                print(f"{Fore.RED}Error, the request file that you provided doesn't exist")
                print_help_menu()
                return

            bypass.headers, bypass.body_content = bypass.parse_req_file(filename)
